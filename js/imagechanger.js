...
function ImageSequence(name, urls,links)
{
 ...
 this.links = links;
 var cursor = 0;
 this.now = function()  {return urls[cursor];};
 this.next = function() {cursor = (cursor+1)%urls.length; return urls[cursor];};
 this.prev = function() {cursor = (cursor-1+urls.length)%urls.length; return urls[cursor];};
 this.displayNext = function() {document.getElementById(name+'_img').src = this.next();document.getElementById(name+'_a').href = this.links[cursor];};
 this.displayPrev = function() {document.getElementById(name+'_img').src = this.prev();document.getElementById(name+'_a').href = this.links[cursor];};
}
...
document.write(
'<div class="imagesequence-container">\n');
if(urls.length>1) {document.write('<div class="imagesequence-prev"><img src="'+leftArrowURL+ '" onClick="imageSequences.'+name+'.displayPrev()"/></div>'+'\n');}
...
document.write('</div>');
if(urls.length>1) {document.write('<div class="imagesequence-next"><img src="'+rightArrowURL+'" onClick="imageSequences.'+name+'.displayNext()"/></div>');}
